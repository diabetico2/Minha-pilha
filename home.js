(() => {
  ...
  panel.innerHTML=`<header class="home-welcome"><div><p class="eyebrow" id="homeGreeting">BEM-VINDO À SUA PILHA</p><h1 id="homeTitle">Toda grande história<br>começa com uma página.</h1><p class="home-description">Quadrinhos, mangás e um lugar para guardar cada leitura.</p><div class="home-hero-actions"><button class="dialog-primary" id="homeContinueBtn">Explorar biblioteca ↓</button><button class="dialog-secondary" id="homeCreateBtn">＋ Criar minha lista</button></div></div><div class="home-feature" id="homeFeature"></div></header>
    ...
    <p class="home-credits">Imagens do catálogo: fontes das ordens de leitura. Personagens e artes pertencem aos respectivos criadores e editoras.</p>`;
  ...
  function refresh() {
    ...
    if(feature){const image=artwork(feature);$('#homeFeature').innerHTML=`<button class="home-feature-button" data-open="${escape(feature.id)}"><img src="${escape(image.url)}" alt="${escape(feature.title)}" referrerpolicy="no-referrer"><span><small>${active?'CONTINUE DE ONDE PAROU':'UMA HISTÓRIA PARA COMEÇAR'}</small><strong>${escape(feature.title.replace(' Reading Order',''))}</strong><b>Abrir leitura ↗</b></span></button>`;$('#homeFeature button').onclick=()=>app.openOrder(feature.id);}
    ...
    $('#homeCards').innerHTML=sorted.slice(0,limit).map(order=>{
      ...
      return `<article class="home-card"><button class="home-card-open" data-open="${escape(order.id)}"><span class="home-card-art"><img src="${escape(image.url)}" alt="" loading="lazy" referrerpolicy="no-referrer"><span class="home-card-tag">${escape(order.personal?'PESSOAL · '+order.family:order.publisher)}</span></span><span class="home-card-copy"><span class="home-card-family">${escape(order.family)}</span><strong>${escape(order.title.replace(' Reading Order',''))}</strong><span class="home-card-progress">${progress.read} de ${progress.total} itens <b>${progress.pct}%</b></span><span class="bar"><span style="width:${progress.pct}%"></span></span></span></button>${order.personal?`<div class="home-card-tools"><button data-edit="${escape(order.id)}">Editar</button><button data-share="${escape(order.id)}">Compartilhar</button></div>`:image.source?`<a class="home-card-credit" href="${escape(image.source)}" target="_blank" rel="noopener noreferrer">Imagem e ordem: ${escape(image.credit)} ↗</a>`:''}</article>`;
    }).join('');
